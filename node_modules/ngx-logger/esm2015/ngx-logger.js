...
class NGXLoggerHttpService {
    ...
    logOnServer(url, log, customHeaders) {
        const /** @type {?} */ headers = customHeaders || new HttpHeaders();
...
class NGXLoggerConfigEngine {
     ...
     */
    _clone(object) {
        const /** @type {?} */ cloneConfig = new LoggerConfig();
...
class NGXLogger {
    ...
    _log(level, message, additional = [], logOnServer = true) {
        ...
        const /** @type {?} */ isLog2Server = logOnServer && config.serverLoggingUrl && level >= config.serverLogLevel;
        const /** @type {?} */ isLog2Console = !(level < config.level);
...
class CustomNGXLoggerService {
    ...
    create(config, httpService, logMonitor) {
        ...
        const /** @type {?} */ logger = new NGXLogger(httpService || this.httpService, config, this.platformId);
        if (logMonitor) {
